...
impl ServiceContainer {
    ...
    fn register<T: Sized + Any + Send + Sync + 'static>(&self, service: T) {
        ...
        println!("register type id:{:?}", type_id);
        println!("register type name:{:?}", any::type_name::<T>());
    ...
    fn resolve<T: ?Sized + Any + Send + Sync + 'static>(&self) -> Option<Arc<T>> {
        let type_id = TypeId::of::<T>();

        println!("resolve type id:{:?}", type_id);
        println!("resolve type name:{:?}", any::type_name::<T>());

        if let Some(service) = self.services.lock().unwrap().get(&type_id) {
            let downcast = service.downcast_ref::<Arc<T>>();

            if downcast.is_none() {
                println!("arc<t> type is not found");
            }

            if downcast.is_some() {
                let aa = downcast.unwrap();
                return Some(aa.clone());
            }
        }

        return self.resolve_trait::<T>();
        ...
        T: ?Sized + Send + Sync + 'static,
    {
        ...
        if let Some(service) = self.services.lock().unwrap().get(&type_id) {
            let downcast = service.downcast_ref::<Arc<T>>().unwrap();
...
fn container_injection_main() {
    ...
    container.register::<Arc<dyn DatabaseService>>(
