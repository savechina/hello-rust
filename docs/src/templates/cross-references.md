...
**示例**:
```markdown
继续之前，请先阅读 [了解所有权](ownership.md)。

这个模式在 [特征](trait.md) 中有进一步扩展。

参考 [结构体](struct.md) 章节了解更多细节。
```
...
```markdown
学习本章前，你需要理解：
- [变量表达式](expression.md) - 变量绑定和基础语法
- [数据类型](datatype.md) - 基本类型和复合类型
- [所有权](ownership.md) - 所有权系统和移动语义
```
...
**完整路径**:
```markdown
查看 [异步编程](../advance/async.md#tokio-运行时) 的 Tokio 运行时部分。
```
